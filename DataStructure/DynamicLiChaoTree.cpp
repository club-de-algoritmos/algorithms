 ...
 * **Cuidado con la memoria dinamica pude causar errores si
 * no se tiene cuidado.
  ...
  ll query_segment(ll x, ll l, ll r, ll L, ll R) {
    ...
    if (l >= L and r <= R) {
      ...
      if (l < r) {
        if (x <= m && left != nullptr)
          ans =
              min(ans, left->query_segment(x, l, m, L, R));
        if (x > m && right != nullptr)
          ans = min(
              ans, right->query_segment(x, m + 1, r, L, R));
      }
      return ans;
    }
    ll ans = inf;
...
struct LiChaoTree {  // the input values for lichaotree are
                     // boundaries of x values you can use
                     // to query with
  int L, R;
  node* root;
  LiChaoTree()
      : L(numeric_limits<int>::min() / 2),
